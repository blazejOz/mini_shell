...
void test_builtin_exit() {
    ...
    assert(builtin_run(BUILTIN_EXIT, argv1, &should_exit, &exit_status) == 0);
    assert(should_exit == 1);

    //test: exit 42
    should_exit = 0; exit_status = 0;
    char *argv2[] = {"exit", "42", NULL};
    assert(builtin_run(BUILTIN_EXIT, argv2, &should_exit, &exit_status) == 0);
    assert(should_exit == 1);
    ...
    assert(builtin_run(BUILTIN_EXIT, argv3, &should_exit, &exit_status) != 0);
    assert(should_exit == 0);

    //test: exit 1 2 -> too many args, don't exit
    should_exit = 0; exit_status = 0;
    char *argv4[] = {"exit", "1", "2", NULL};
    assert(builtin_run(BUILTIN_EXIT, argv4, &should_exit, &exit_status) != 0);
    assert(should_exit == 0);
}
...
void test_builtin_pwd() {
    //test: pwd
    char *argv[] = {"pwd", NULL};
    assert(builtin_run(BUILTIN_PWD, argv, NULL, NULL) == 0);

    //test: too many args
    char *argv2[] = {"pwd", "extra", NULL};
    assert(builtin_run(BUILTIN_PWD, argv2, NULL, NULL) != 0);
}
...
void test_builtin_cd(){
    ...
    char *argv1[] = {"cd", NULL};
    assert(builtin_run(BUILTIN_CD, argv1, NULL, NULL) == 0);
    ...
    assert(builtin_run(BUILTIN_CD, argv2, NULL, NULL) == 0);
    ...
    assert(builtin_run(BUILTIN_CD,  argv3, NULL, NULL) != 0);

    // cd into non-existent dir
    char *argv4[] = {"cd", "/this_does_not_exist", NULL};
    assert(builtin_run(BUILTIN_CD, argv4, NULL, NULL) != 0);
